...
bool loadMedia()
{
	...
	//Open the font
	gFont = TTF_OpenFont( "../data/33/lazy.ttf", 28 );
	if( gFont == NULL )
	{
	...
	SDL_RWops* file = SDL_RWFromFile( "../data/33/nums.bin", "r+b" );
	...
	if( file == NULL )
	{
		...
		file = SDL_RWFromFile( "../data/33/nums.bin", "w+b" );
		if( file != NULL )
		{
...
void close()
{
	...
	SDL_RWops* file = SDL_RWFromFile( "../data/33/nums.bin", "w+b" );
	if( file != NULL )
	{
		//Save data
		for( int i = 0; i < TOTAL_DATA; ++i )
